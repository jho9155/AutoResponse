	...
	case WM_COMMAND:
		...
		case IDC_BUTTON_RXETC:
			...
			for (i = 0; i < lstrlen(ETCTEMP); i++) {
				if ((ETCTEMP[i] < 48) && (ETCTEMP[i] > 57) || (ETCTEMP[i] != ',')) MessageBox(hDlg, _T("Please Input Integer\nex)13,10"), _T("ERROR"), MB_OK);
			}
		...
		case IDC_BUTTON_TXETC:
			...
			for (i = 0; i < lstrlen(ETCTEMP); i++) 	{
				if (((ETCTEMP[i] < 48) || (ETCTEMP[i] > 57)) && (ETCTEMP[i] != ',')) {
...
BOOL CALLBACK Dlg_FileConfig(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON_RES_ADD:
			...
			STR_SEP_CHAR(Temp, '|', Buffer1, Buffer2, 128);
			LI.pszText = Buffer1;
			g_Cfg[gSendStep].Send[g_Cfg[idx].SendCnt-1].index = _tstoi(Buffer1);
			...
			LI.iSubItem = 2;
			pszData = Buffer2;
			g_Cfg[gSendStep].Send[g_Cfg[idx].SendCnt-1].Delay = _tstoi(Buffer2);
